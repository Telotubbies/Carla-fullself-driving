...
class AttentionLSTM(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int, num_layers: int, output_size: int = 4, sequence_length: int = 10):
        super().__init__()
    ...
    def get_sequence_length(self) -> int:
        """Get required sequence length (for ITemporalModule interface)."""
    ...
    def predict(self, sequence: np.ndarray) -> np.ndarray:
        ...
        Implements ITemporalModule.predict() interface.
        
        Args:
            sequence: Feature sequence (sequence_length, feature_dim) as numpy array
        
        Returns:
            Predicted state (4,) [x, y, yaw, velocity] as numpy array
        ...
        if sequence.ndim != 2:
            raise DataValidationError(f"Sequence must be 2D, got {sequence.ndim}D")
        
        expected_length = self.sequence_length
        if sequence.shape[0] != expected_length:
            raise DataValidationError(
                f"Sequence length mismatch: expected {expected_length}, got {sequence.shape[0]}"
            )
        ...
        with torch.no_grad():
            try:
                ...
                if output.shape != (self.output_size,):
                    raise DataValidationError(
                        f"Output shape mismatch: expected ({self.output_size},), got {output.shape}"
                    )
                
                # Validate for NaN/Inf
                if np.any(np.isnan(output)) or np.any(np.isinf(output)):
                    raise DataValidationError("Prediction output contains NaN or Inf values")
