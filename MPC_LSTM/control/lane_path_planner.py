...
class LanePathPlanner:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.lookahead_distance = 30.0  # meters (increased for better planning)
        self.lane_width = 3.5  # meters (typical lane width)
        self.lateral_offset = 0.0  # meters (0 = center, positive = right)
        
        # Smoothing parameters
        self.smoothing_factor = 0.2  # Less smoothing for more responsive control
    ...
    def _generate_from_waypoints(
        ...
        """Generate trajectory from CARLA waypoints with proper spacing for MPC."""
        x, y, yaw, v = current_state
        
        target_v = max(5.0, min(12.0, v + 1.0)) if v > 0.5 else 5.0
        
        ref_traj = np.zeros((self.horizon + 1, 4))
        
        # Collect dense waypoints along the road
        wp_points = []
        current_wp = waypoints[0] if len(waypoints) > 0 else None
        
        if current_wp is None:
            return self._generate_straight_ahead(current_state)
        
        # Generate waypoints with fine spacing (0.5m) for smooth interpolation
        for _ in range(100):
            if current_wp is None:
                break
            wp_loc = current_wp.transform.location
            wp_rot = current_wp.transform.rotation
            wp_points.append((wp_loc.x, wp_loc.y, np.deg2rad(wp_rot.yaw)))
            next_wps = current_wp.next(0.5)
            current_wp = next_wps[0] if next_wps and len(next_wps) > 0 else None
        
        if len(wp_points) < 2:
            return self._generate_straight_ahead(current_state)
        
        wp_arr = np.array(wp_points)
        
        # Compute cumulative distances along waypoints
        diffs = np.diff(wp_arr[:, :2], axis=0)
        segment_lengths = np.sqrt(np.sum(diffs**2, axis=1))
        cum_dist = np.concatenate([[0.0], np.cumsum(segment_lengths)])
        
        # Find closest point on waypoint path to current position
        dists_to_vehicle = np.sqrt((wp_arr[:, 0] - x)**2 + (wp_arr[:, 1] - y)**2)
        closest_idx = np.argmin(dists_to_vehicle)
        start_dist = cum_dist[closest_idx]
        
        # Generate trajectory points at dt intervals
        for i in range(self.horizon + 1):
            speed_i = v + (target_v - v) * (i / (self.horizon + 1))
            travel_dist = start_dist + speed_i * self.dt * i
            
            # Interpolate along waypoint path
            if travel_dist >= cum_dist[-1]:
                last_yaw = wp_arr[-1, 2]
                overshoot = travel_dist - cum_dist[-1]
                ref_traj[i, 0] = wp_arr[-1, 0] + overshoot * np.cos(last_yaw)
                ref_traj[i, 1] = wp_arr[-1, 1] + overshoot * np.sin(last_yaw)
                ref_traj[i, 2] = last_yaw
            else:
                idx = np.searchsorted(cum_dist, travel_dist) - 1
                idx = max(0, min(idx, len(wp_arr) - 2))
                alpha = (travel_dist - cum_dist[idx]) / max(segment_lengths[idx], 1e-6)
                alpha = np.clip(alpha, 0.0, 1.0)
                
                ref_traj[i, 0] = wp_arr[idx, 0] + alpha * (wp_arr[idx+1, 0] - wp_arr[idx, 0])
                ref_traj[i, 1] = wp_arr[idx, 1] + alpha * (wp_arr[idx+1, 1] - wp_arr[idx, 1])
                
                yaw_diff = wp_arr[idx+1, 2] - wp_arr[idx, 2]
                yaw_diff = np.arctan2(np.sin(yaw_diff), np.cos(yaw_diff))
                ref_traj[i, 2] = wp_arr[idx, 2] + alpha * yaw_diff
            
            ref_traj[i, 3] = speed_i
