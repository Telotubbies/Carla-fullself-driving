"""
Exploratory Data Analysis (EDA) for IL Demonstration Data
วิเคราะห์และ visualize ข้อมูล IL demonstrations
"""
...
def analyze_basic_stats(data):
    ...
    # Separate action components (if available)
    if 'steers' in data:
    ...
    if 'velocities' in data:
        ...
        speeds_3d = np.linalg.norm(velocities, axis=1) * 3.6  # Convert to km/h
        print(f"\n⚡ Velocities (3D, km/h):")
...
def plot_speeds(data, output_dir='plots'):
    ...
    # Speed over time (first 1000 steps)
...
def plot_vehicle_state(data, output_dir='plots'):
    """Plot vehicle state information"""
    os.makedirs(output_dir, exist_ok=True)
    ...
    if 'velocities' in data:
        velocities = data['velocities']
        speed_3d = np.linalg.norm(velocities, axis=1) * 3.6  # km/h
